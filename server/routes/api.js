...
// const dbFirstSetup = require('../modules/dbFirstSetup')

router.get("/getVideo/:sentence", async (req, res) => {
  
  let wordsLookupPromises = []
  let    dbSearchPromises = []
  let    dbUpdatePromises = []
  let        apiPromisess = []
  let   masterWordsData = [] // variable that holds the data from E2E
  
  const sentenceToBuild = req.params.sentence
  const wordsToLookup = sentenceToBuild.split(' ')
  ...
  const isMasterReadyVideoId = async function(){
    ...
    if (isEmpty) { 
      const sendToApi = masterWordsData.filter( object => object.videoIds.length == 0)
  ...
  console.log('efrati')
  const foundSearchedWords = await Promise.all(wordsLookupPromises)
  ...
  const retrieveIdsFromAPI = async function (wordDataObj) {
    ...
    const resolvedApiPromises = await Promise.all(apiPromisess)
