...
let times = [] // all timestamps in transcript
const timeData = [] // all relevant time info for each word
const firstDbUpdatePromises = []
let downloadCommands = []
let scripts = [] // storing all of the normalized data for each script
let relevantObjects = []
let youtubeVideoIDs = []


const scriptsFolder = '/Users/vicki/Documents/frienerator/Frienerator2.0/server/modules/youtube_transcripts'
...
const retrieveTimeStamppData = async function (masterDataArray) {
    ...
    const runCommand = function () {
        ...
        doExist.forEach((videoIdExists, i) => {
            if (!videoIdExists) {
    ...
    dbUpdatePromises.push(searchedWord.updateMany(
        {
            this: { $elemMatch: { matchedEpisodes: { $size: 2 } } },
            // $.matchedEpisodes.length: { $size: 2 },
